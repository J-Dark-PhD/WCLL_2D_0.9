...
def Cp_W(T, main=False):  # units in J/(kg*K)
    return 21.868372 + 8.068661e-03*T - 1e-06*T**2 + 1.075862e-09*T**3 + 1.406637e04/T**2
...
def rho_W(T, main=False):  # units in kg/m**3 
    ...
    """
    T_W_0 = 293.15
    return 19250 - 2.66207e-01*(T - T_W_0) - 3.0595e-06*(T - T_W_0)**2 - 9.5185e-09*(T - T_W_0)**3


def thermal_cond_W(T, main=False):  # units in W/(m*K)
    return 149.441 - 45.466e-03*T + 13.193e-06*T**2 - 1.484e-09*T**3 + 3.866e06/(T+1)**2
...
S_0_eurofer_esteban = 5.5463e21  # Solubility coefficient pre-exponential factor (atom/m3 Pa^-0.5)
...
S_0_eurofer = 2.4088e23  # Solubility coefficient pre-exponential factor (atom/m3 Pa^-0.5)
...
S_0_eurofer_aiello = 6.1424e22  # Solubility coefficient pre-exponential factor (atom/m3 Pa^-0.5)
...
def visc_lipb(T, main=False):  # units (Pa s)
    ...
    return 0.01555147189 - 4.827051855e-05*T + 5.641475215e-08*T**2 - 2.2887e-11*T**3
...
# ##### Literature review
# taken from (Reiter, 1990)
S_0_lipb_reiter_H = 4.4310e+20   # Solubility coefficient pre-exponential factor
E_S_lipb_reiter_H = 0.01399      # Solutbiility coefficient activation energy (eV)

S_0_lipb_reiter_D = 4.2857e+20  # Solubility coefficient pre-exponential factor
E_S_lipb_reiter_D = 0.01399     # Solutbiility coefficient activation energy (eV)

S_0_lipb_reiter_T = 4.2131e+20   # Solubility coefficient pre-exponential factor
...
if __name__ == "__main__":
    ...
    plt.plot(1000/T, S_eurofer_esteban(T, main=True), label="Eurofer (Esteban, 2007)", color='red')
    plt.plot(1000/T, S_eurofer(T, main=True), label="Eurofer (Chen, 2021)", color='green')
    plt.plot(1000/T, S_eurofer_aiello(T, main=True), label="Eurofer (Aiello, 2002)", color='black')
